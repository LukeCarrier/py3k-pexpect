...
def main():
    ...
    ip_list = dict([ (key,value) for key,value in list(ip_list.items()) if '192.168.' not in key])
    ip_list = dict([ (key,value) for key,value in list(ip_list.items()) if '127.0.0.1' not in key])
